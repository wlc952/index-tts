...
class OnnxConverter(BaseConverter):
    ...
    def convert_conv_op(self, onnx_node):
        ...
        # Sanity check for grouped/depthwise conv to catch wrong weight mapping early
        try:
            if self.isWeight(onnx_node.inputs[1]):
                wname = onnx_node.inputs[1]
                wshape = self.getShape(wname)
                # ONNX Conv weights: [oc, ic/group, k] for 1D, [oc, ic/group, kh, kw] for 2D
                if isinstance(wshape, (list, tuple)) and len(wshape) >= 2:
                    oc = int(wshape[0])
                    ic_per_group = int(wshape[1])
                    if group and int(group) > 1:
                        if oc == 1 and ic_per_group == int(group):
                            # Apply scheme A: transpose weight from [1, C, ...] to [C, 1, ...] for depthwise conv
                            # Get original weight and transpose it
                            w_orig = self.getWeight(wname)
                            if len(wshape) == 3:  # 1D conv: [1, C, K] -> [C, 1, K]
                                w_fixed = w_orig.transpose(1, 0, 2)
                            elif len(wshape) == 4:  # 2D conv: [1, C, KH, KW] -> [C, 1, KH, KW]
                                w_fixed = w_orig.transpose(1, 0, 2, 3)
                            else:
                                raise RuntimeError(f"Unsupported weight shape for transpose: {wshape}")
                            
                            # Create unique weight name to avoid conflicts with ConvTranspose
                            safe_node = onnx_node.name.replace('/', '_').replace('.', '_')
                            new_wname = f"{wname}__dwfix__{safe_node}"
                            
                            # Add the transposed weight
                            self.addWeight(new_wname, w_fixed.astype(w_orig.dtype, copy=False))
                            
                            # Use the new weight for this conv - IMPORTANT: update filter_op, not operands
                            filter_op = self.getWeightOp(new_wname)
                            
                        elif oc % int(group) != 0 or ic_per_group != 1:
                            raise RuntimeError(
                                f"Invalid grouped Conv weights for node '{onnx_node.name}': "
                                f"group={group}, weight_shape={wshape}, kernel_shape={kernel_shape}. "
                                f"Expected oc % group == 0 and ic/group == 1 for depthwise/group conv.")
        except Exception as e:
            # Re-raise with clearer context
            raise RuntimeError(f"Conv node '{onnx_node.name}' weight validation failed: {e}")
