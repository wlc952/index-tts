...
class IndexTTS:
    ...
    def infer(self, audio_prompt, text, output_path, verbose=False):
        ...
        if self.cache_cond_mel is None or self.cache_audio_prompt != audio_prompt:
            ...
            fix_mel_tokens = 300
            cond_mel = cond_mel[..., :fix_mel_tokens]  # 截断多余部分
            if cond_mel.shape[-1] < fix_mel_tokens:
                print("padded cond_mel")
                pad_len = fix_mel_tokens - cond_mel.shape[-1]
            ...
            with torch.no_grad():
                ...
                with torch.amp.autocast(text_tokens.device.type, enabled=self.dtype is not None, dtype=self.dtype):
                    ...
                    if debug:
                        ...
                        np.savez('latent_debug.npz', latent=latent_np, original_length=ori_)
                        latent = torch.nn.functional.pad(latent, (0, 0, 0, 224 - latent.size(1)), value=0.0)  # pad to max length
                    wav, _ = self.bigvgan(latent, auto_conditioning.transpose(1, 2))
                    bigvgan_time += time.perf_counter() - m_start_time
                    wav = wav.squeeze(1)
...
if __name__ == "__main__":
    ...
    tts = IndexTTS(cfg_path="checkpoints/config.yaml", model_dir="checkpoints", is_fp16=False, use_cuda_kernel=False, device="cpu")
